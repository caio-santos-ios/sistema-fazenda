...
namespace api_bora_trampar.src.Controllers
{
    [ApiController]
    [Authorize]
    [Route("api/group-sub-cost-centers")]
    public class GroupSubCostCenterController(IGroupSubCostCenterService service) : ControllerBase
    {
        [HttpGet]
        ...
        {
            ResponseApi<GroupSubCostCenter?> response = await service.GetByIdAsync(id);
            return StatusCode(response.StatusCode, new { response.Result });
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CreateGroupSubCostCenterRequest request)
        {
            ...
            ResponseApi<GroupSubCostCenter?> response = await service.CreateAsync(request);
            return StatusCode(response.StatusCode, response.StatusCode == 201 ? new { response.Result } : new { message = response.Message });
        }

        [HttpPut]
        public async Task<IActionResult> Update([FromBody] UpdateGroupSubCostCenterRequest request)
        {
            if (request == null) return BadRequest("Dados inválidos.");

            request.UpdatedBy = User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "";

            ResponseApi<GroupSubCostCenter?> response = await service.UpdateAsync(request);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            ResponseApi<GroupSubCostCenter?> response = await service.DeleteAsync(new() { Id = id, DeletedBy = userId });
